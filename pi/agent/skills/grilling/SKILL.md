...
Work the tree in **rounds**. The **frontier** is every decision whose prerequisites are already settled: the questions you can ask _now_ without guessing at answers you haven't heard yet. Ask the whole frontier in one `ask_user` call: number each question and give your recommended answer. Every question in that call must be answerable without knowing another answer from the same call. Then wait for the user's answers before the next round.

Before the first `ask_user` call in every grilling session, read [`QUESTION-DESIGN.md`](QUESTION-DESIGN.md). Before every call, run its ordered gates and send only items that pass. Read it again when the user reports confusion.

Each round the user answers reshapes the tree: settled decisions push the frontier outward and unblock questions that depended on them. Recompute the frontier and ask the next round. A question whose answer depends on another question still open in this round belongs to a _later_ round, not this one. If the user says they do not understand or asks for tradeoffs, keep that decision unsettled. Identify the missing context, rewrite the item from the user's viewpoint, and split it if the original combined separate decisions.
